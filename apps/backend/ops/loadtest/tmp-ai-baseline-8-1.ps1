...
foreach($r in 1..5){
 foreach($t in $targets){
  ...
  }catch{ $sw.Stop(); $rows += [pscustomobject]@{flow=$t.flow;instance=($t.url.Substring(0,21));status=0;latencyMs=[int]$sw.ElapsedMilliseconds;fallbackUsed=$null;outChars=0} }
 }
}
$summary = $rows | Group-Object flow | ForEach-Object {
 ...
 [pscustomobject]@{flow=$_.Name;calls=$count;avgMs=$avg;p50Ms=$p50;p95Ms=$p95;maxMs=$max;status200=(($_.Group|Where-Object status -eq 200).Count);fallbackTrue=(($_.Group|Where-Object fallbackUsed -eq $true).Count)}
}
[pscustomobject]@{summary=$summary;sample=$rows[0..5]} | ConvertTo-Json -Depth 6
